        ...
        public Room GetById(int id)
        {
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    ...
                    // @id is the SQL parameter. AddWithValue links the SQL parameter with the method parameter
                    cmd.Parameters.AddWithValue("@id", id);
                    SqlDataReader reader = cmd.ExecuteReader();

                    // Instantiates room with a value null
                    Room room = null;

                    // If we only expect a single row back from the database, we don't need a while loop.
                    // Reads the Room table for a match by ID.
                    // If a record exists, room is updated with new object
                    if (reader.Read())
                    {
                        room = new Room
                        {
                    ...
                    // Connection closes regardles of whether a record is or isn't found
                    reader.Close();
